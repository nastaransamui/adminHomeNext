...
const SortBy = forwardRef((props, ref) => {
  ...
  const { t, rtlActive, dataFields } = props;
  const [anchorEl, setAnchorEl] = useState(null);
  const open = Boolean(anchorEl);
  const id = open ? 'sort-by-popover' : undefined;
  const dispatch = useDispatch();
  const {usersSortBy,  usersCardView } = useSelector((state) => state);
                  ...
                  return (
                    <Fragment key={index}>
                      <ListItem
                        ...
                        <ListItemButton>
                          <ListItemIcon>
                            {usersSortBy.field == anchorSTl[`filed`] &&
                            ((usersSortBy.sorting == 1 && list == 'ASC') ||
                              (usersSortBy.sorting == -1 && list == 'DESC')) ? (
                              <Check color='primary' />
                            ) : null}
                          </ListItemIcon>
                          <ListItemText
